...
public class Explosion {
                ...
                if (grid[i][j].getIndex() == (byte) 3) {
                    ...
                    if (!grid[i][j].getMarker()) {
                        timer.schedule(new java.util.TimerTask() {
                            @Override
                            public void run() {
                                explosionAllDirections(ipos, jpos, (byte) 4, grid, player, false);
                                timer.schedule(new java.util.TimerTask() {
                                    @Override
                                    public void run() {
                                        explosionAllDirections(ipos, jpos, (byte) 0, grid, player, true);
                ...
                try {
                    if(grid[ipos + i * x] [jpos + i * y].getIndex() == 4)
                    {
                        ...
                        if(grid[ipos + i * x] [jpos + i * y].getPowerUpMarker() != 0 && index == 0) {
                            grid[ipos + i * x] [jpos + i * y].setIndex((byte)checkPowerUpIndex
                                    (grid, ipos + i *x, jpos + i * y));
                        }
                    }}
                catch(Exception e)
                {
    ...
    public void explosionAllDirections(int ipos, int jpos, byte index, Tile[][] grid, Player player, boolean secondCall) {
